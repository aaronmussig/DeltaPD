...
pub fn run_deltapd(qry_mat: &QryDistMatrix, ref_mat: &RefDistMatrix, metadata_file: &MetadataFile, params: &Params) -> DeltaPDResult<Vec<OutputResult>> {
    ...
    // let mut output = Vec::with_capacity(knn_qry.len());
    ...
    //
    // // Iterate over the query taxa and their k-nearest neighbours
    // println!("Iterating over each taxon");
    // let previous_time = Instant::now();
    // for (q_taxon, _qry_knn_idx) in &knn_qry {
    //
    ...
    //     //
    //     // println!("Removing {}", q_taxon.0);
    //     // println!("{:?}", std_error);
    //     // println!("{:?}", knn_as_linalg.ref_labels);
    //     // println!("{:?}", knn_as_linalg.qry_labels);
    //     output.push(OutputResult {
    //         linear_model_base,
    //         linear_model_jk: jackknife_models,
    //         relative_influence,
    //         std_error,
    //         taxon_removed: q_taxon.clone(),
    //         knn_as_linalg,
    //     });
    // }
